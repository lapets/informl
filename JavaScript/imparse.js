...
(function (_, uxadt_old) {
  ...
  I.tokenize = function (grammar, str) {
    ...
    var cbs = _.flatten(uxadt_old(ps).map(I.Production(_, _), function (nt, cbs) { return cbs; })); // Choice blocks
    var cs = _.flatten(uxadt_old(cbs).map(I.Choices(_), function (cs) { return cs; })); // Choices
    var es = _.flatten(uxadt_old(cs).map(I.Choice(_, _, _), function (l, a, seq) { return seq; })); // Choice sequences
  ...
  I.parse = function (grammar, tmp, nt, lf) {
    ...
    return uxadt_old(ps).find(I.Production(_, _), function (pnt, choiceBlocks) { // Select appropriate production.
      if (pnt != nt && nt != null) {
        // Skip production.
      } else {
        return uxadt_old(choiceBlocks).find(I.Choices(_), function (choices) { // Select appropriate choice block.
          return uxadt_old(choices).find(I.Choice(_, _, _), function (label, a, seq) { // Select appropriate choice.
  ...
  ///////////////////////////////////////////////////////////////////
  // Pretty print uxadt_old grammar for Javascript.
  I.printuxadt_oldGrammar = function (grammar, indent, synonym) {
    ...
    var printuxadt_oldSeq = function(seq, depth) {
      ...
      for (var i = 0; i < seq.length; i++) {
        if (_.isArray(seq[i])) {
          str += printuxadt_oldSeq(seq[i], depth);
        } else {
          var e = seq[i]
          ._(I.Terminal(_), function (t) {
          ...
          ._(I.One(_), function (inseq) {
            if (inseq.length < 3)   return synonym + ".One([ " + printuxadt_oldSeq(inseq) + " ])"; 
            return synonym + ".One([ " + space + indent + printuxadt_oldSeq(inseq, depth+1) + " ])"; 
          })
          ._(I.May(_), function (inseq) {
            if (inseq.length < 3)   return synonym + ".May([ " + printuxadt_oldSeq(inseq) + " ])";
            return synonym + ".May([ " + space + indent + printuxadt_oldSeq(inseq, depth+1) + " ])";
          })
          ._(I.Many(_), function (inseq) {
            if (inseq.length < 3)   return synonym + ".Many([ " + printuxadt_oldSeq(inseq) + " ])";
            return synonym + ".Many([ " + space + indent + printuxadt_oldSeq(inseq, depth+1) + "])";
          })
          ._(I.MayMany(_), function (inseq) {
            if (inseq.length < 3)   return synonym + ".MayMany([ " + printuxadt_oldSeq(inseq) + " ])";
            return synonym + ".MayMany([ " + space + indent + printuxadt_oldSeq(inseq, depth+1) + " ])";
          })
          .end;
          es.push(e);
  ...
  ///////////////////////////////////////////////////////////////////
  // BNF -> uxadt_old
  // Regular expressions needed to tokenize and/or parse BNF to uxadt_old.
  ...
  // Grammar definition for converting BNF to uxadt_old.
  ...
  // Converts BNF grammar (in text) into a uxadt_old grammar.
  I.bnfTouxadt_old = function (bnf) {
    var tokens = I.bnfTokenize(bnf);
    var pt = I.parse(I.genGrammar, tokens);
    if (pt != null) {
      if (pt[1].length == 0) {
        var ux = I.bnfTreeTouxadt_old(pt[0]);
        return ux;
      } else {
  ...
  // Converts a BNF parse tree and transforms it into uxadt_old.
  I.bnfTreeTouxadt_old = function (bnftree) {
    var ps = bnftree["Grammar"];
    var ux = [];

    // Helper function to convert expressions into uxadt_old.
    var touxadt_oldSeq = function (es) {
      var cs = [];
      var key = _.keys(es)[0];
      var value = es[key];
      ...
      switch (key) {
        ...
        case "Group":
          for (var i = 0; i < value.length; i++) {
            cs.push(touxadt_oldSeq(value[i]));
          }
          break;
      }
      return cs;
