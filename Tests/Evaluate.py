...
def evalTerm (term, env):
  ...
  if __iml0 < Test.Plus(None,None):
    (a,b,) = __iml0
    x = evalTerm(a, env)# Local variable.
    y = evalTerm(b, env)# Local variable.
...
def execute (prog):
  ...
  if __iml22 < Test.Program(None):
    (ss,) = __iml22
    out = Informl.map(ss, lambda x : evalStatement(x, env))# Local variable.
    ret = []# Local variable.
    for o in out:
      if o != None:
